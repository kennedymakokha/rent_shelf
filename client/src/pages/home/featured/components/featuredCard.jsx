    ...
    return (
        <Link state={props.featured} to={`/shelves/${props?.featured?.title.replace(/\s+/g, "-").toLowerCase()}`} className="sm:w-1/5 w-full  h-full group shrink-0  p-2 flex justify-center items-center">
            ...
            <div className="bg-gray-400 w-full h-full relative z-0 rounded-md shadow-2xl">
                ...
                {/*  */}
                {!props.hide && <Bottom text={props.text} bg={props.bg} featured={props.featured} />}

                {props.featured.featured && < div className="absolute  bg-black flex-col gap-y-2 transition duration-50 fade-in opacity-80 hidden top-0  w-full h-[64%] group-hover:flex justify-center px-0 items-center z-20  ">
                    <ul className='px-2'>
                ...
                <Pricebadge noPrice={props.noPrice} feature={props.feature} opp={props.opp} featured={props?.featured} />
